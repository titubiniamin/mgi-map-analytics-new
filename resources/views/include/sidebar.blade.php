...
@php
    use Illuminate\Support\Facades\Auth;$usr = Auth::guard('web')->user();
@endphp
<div class="app-sidebar colored">
    <div class="sidebar-header">
        <a class="header-brand" href="{{route('dashboard')}}">
            <div class="logo-img">
               <img height="50" src="{{ asset('img/logo_white.png')}}" class="header-brand-img" title="MGI">
            </div>
        </a>
    ...
    <div class="sidebar-content" >
        <div class="nav-container" style="background: white;">
            <nav id="main-menu-navigation" class="navigation-main">
                ...
                @if($user->can('manage_user') || $user->can('manage_roles') || $user->can('manage_permission'))
                ...
                <!---Inventory Start-->
                @if($user->can('inventory_item_create') || $user->can('inventory_item_update'))
                    <div class="nav-item {{ (Route::is('inventory.item.index')  || Route::is('inventory.item.update') || Route::is('inventory.item.create')) ? 'active open' : '' }} has-sub">
                        <a href="#"><i class="ik ik-shopping-cart"></i><span>{{ __('Inventory')}}</span></a>
                        <div class="submenu-content">
                            @can('inventory_item_update')
                                <a href="{{route('inventory.item.index')}}" class="menu-item {{ Route::is('inventory.item.index')  ? 'active' : '' }}">{{ __('Items')}}</a>
                            @endcan
                            @can('inventory_item_create')
                                <a href="{{route('inventory.item.create')}}" class="menu-item {{ Route::is('inventory.item.create')  ? 'active' : '' }}">{{ __('Add Items')}}</a>
                            @endcan
                            <!-- only those have manage_role permission will get access -->
...
<!---dealer--->
                @if ($usr->can('dealer.create') || $usr->can('dealer.view') || $usr->can('dealer.edit') || $usr->can('dealer.delete') || $usr->can('dealer.import-show') || $usr->can('dealer.import'))
                    <div class="nav-item {{ (Route::is('dealers.index') || Route::is('dealers.create') || Route::is('admin.dealers.edit') || Route::is('dealers.import-show')) ? 'active open' : '' }} has-sub">
                        <a href="#"><i class="fa fa-briefcase"></i><span>{{ __('Dealers') }}</span></a>
                        <div class="submenu-content">
                            @if ($usr->can('dealer.view'))
                                <a href="{{ route('dealers.index') }}" class="menu-item {{ Route::is('dealers.index') || Route::is('dealers.edit') ? 'active' : '' }}">
                                    {{ __('All Dealers') }}
                                </a>
                            @endif
                            @if ($usr->can('dealer.create'))
                                <a href="{{ route('dealers.create') }}" class="menu-item {{ Route::is('dealers.create') ? 'active' : '' }}">
                                    {{ __('Create Dealer') }}
                                </a>
                            @endif
                            @if ($usr->can('dealer.import-show'))
                                <a href="{{ route('dealers.import-show') }}" class="menu-item {{ Route::is('dealers.import-show') ? 'active' : '' }}">
                                    <i class="fa fa-map"></i> {{ __('Excel Import') }}
                                </a>
                            @endif
                        </div>
                    </div>
    ...
    <style>
    .app-sidebar .sidebar-content .nav-container {
        background: #F6F7FB!important; /* Already white in your code */
    }

    .app-sidebar .nav-container a {
        color: black !important; /* Ensure text is black */
    }

    .app-sidebar .nav-container a:hover {
        color: #333 !important; /* Slightly darker for hover effect */
    }

    .app-sidebar .nav-container .active > a {
        color: black !important; /* Ensure active text remains black */
    }
    .app-sidebar .submenu-content {
        background-color: #0076c0 !important; /* Blue background */
    }

    /* Submenu items text color */
    .app-sidebar .submenu-content .menu-item {
        color: white !important; /* White text for better contrast */
    }

    /* Submenu items hover background and text color */
    .app-sidebar .submenu-content .menu-item:hover {
        background-color: darkblue !important; /* Darker blue on hover */
        color: white !important; /* Ensure text stays readable */
    }

    /* Active submenu item background and text color */
    .app-sidebar .submenu-content .menu-item.active {
        background-color: navy !important; /* Navy background for active item */
        color: white !important; /* Ensure active item text is white */
    }
    .sidebar-header{
